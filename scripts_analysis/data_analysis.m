...
elecnum = 8;
EMG_Data = {};
cd('R:\Documents\Git\Myo_Data_Analysis\data\Gestures');
folders = dir();
...
for gestnum = 1: numel(folders)
    ...
    dirname = ['R:\Documents\Git\Myo_Data_Analysis\data\Gestures\' foldernamesch{gestnum}...
    ...
    for volunteer = 1:numvols
        ...
        for elec = 1:elecnum %filenum{volunteer}
            for samp = 1:filenum{gestnum}{volunteer}
                ...
                subplot(4,2,elec)
                title(['Electrode ',num2str(elec)]);
                plot(Interp_Elec{gestnum}{volunteer}{samp}(:,elec),'color',cc(volunteer,:))
                hold off;
            end
            xlabel('Time (ms)')
            ylabel('Amplitude')
